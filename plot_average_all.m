function plot_average_all(runplot,freq,avg_freq_stable,avg_freq_switch)
  ...
  %Edited 2017-11-21. Need to plot trial-based. 
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  case 'tmap'
    %Should decide what to average over. Either timepoints for topo or channels for TFR

    if strcmp(tmap_dim,'time')


      toi1 = find(round(freq.time,2)==round(runplot.timewindow(1),2));
      toi2 = find(round(freq.time,2)==round(runplot.timewindow(2),2));

      [~,tmap]=ttest(squeeze(nanmean(avg_freq_switch(:,:,:,toi1:toi2),4)),squeeze(nanmean(avg_freq_stable(:,:,:,toi1:toi2),4)),'dim',1);
      ...
      caxis([0 0.5])
      colorbar
      title('P values for ttest')

      namefigure='tmap_lowfreqs_TFR';
    elseif strcmp(tmap_dim,'sensor')


      idx_occ=strfind(freq.label,'O');
      idx_occ=find(~cellfun(@isempty,idx_occ));

      [~,tmap]=ttest(squeeze(nanmean(avg_freq_switch(:,idx_occ,:,:),2)),squeeze(nanmean(avg_freq_stable(:,idx_occ,:,:),2)),'dim',1);
