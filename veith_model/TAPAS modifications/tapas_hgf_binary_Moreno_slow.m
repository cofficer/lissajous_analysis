...
if ~isempty(varargin) && strcmp(varargin{1},'trans');
    p = tapas_hgf_binary_transp_Wilson(r, p);
end
...
% Pass through representation update loopl
ambiguous_trials = find(u(:,3) == 0.5);
ambiguous_start_trials = ambiguous_trials([1; find(diff(ambiguous_trials)~=1)+1]);
duration_ambiguous_block = unique(diff(ambiguous_start_trials)/2);
...
for k = 2:1:n
    if not(ismember(k-1, r.ign))
        ...
        if any(k == ambiguous_start_trials)
            ...
            clear delta_r noise_term percept timepoint
            if u(k,1) == 1
                delta_r(1) = 0.05;
            else
                delta_r(1) = -0.05;
            ...
            noise_term(1) = randn(1,1)*noise_variance;
            
            for time = 2:(u(k+duration_ambiguous_block-1,4)-u(k,4)) * 1000
                noise_term(time) = noise_term(time-1) - noise_term(time-1)/tau_s + sigma*sqrt(2/tau_s)*randn(1,1)*noise_variance;
                delta_r(time) = delta_r(time-1) + -4 * delta_r(time-1)*((delta_r(time-1))^2-1) ...
                    - 2*g_a*(delta_r(time-1)-1) - 2*g_b*(delta_r(time-1)+1) ...
                    - noise_term(time); 
            end
            delta_r = zscore(delta_r);
            
            for time = 1:(u(k+duration_ambiguous_block-1,4)-u(k,4)) * 1000
                if delta_r(time) > boundary
                    percept(1,time) = 1;
                elseif delta_r(time) < -boundary
                    percept(1,time) = 0;
                else
                    percept(1,time) = percept(1,time-1);
                end
            ...
            for idx = k:k+duration_ambiguous_block-1
             ...
             timepoint =  round((u(idx,4)-u(k,4)) * 1000);
             timepoint(timepoint == 0) = 1;
      
            predicted(idx,1) = percept(1, timepoint);
