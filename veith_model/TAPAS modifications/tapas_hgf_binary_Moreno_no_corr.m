...
for k = 2:1:n
    if not(ismember(k-1, r.ign))
        ...
        if any(k == ambiguous_start_trials)
            ...
            noise_term(k) = randn(1,1)*noise_variance;
            
            for idx = k:k+duration_ambiguous_block-1
                noise_term(idx) = noise_term(idx-1) - noise_term(idx-1)/tau_s + sigma*sqrt(2/tau_s)*randn(1,1)*noise_variance;
                delta_r(idx) = delta_r(idx-1) + -4 * delta_r(idx-1)*((delta_r(idx-1))^2-1) ...
                    - 2*g_a*(delta_r(idx-1)-1) - 2*g_b*(delta_r(idx-1)+1) ...
                    - noise_term(idx); 
            end
            delta_r(k:k+duration_ambiguous_block-1) = zscore(delta_r(k:k+duration_ambiguous_block-1));
            
            for idx = k:k+duration_ambiguous_block-1
                if delta_r(idx) > boundary
                    predicted(idx,1) = 1;
                elseif delta_r(idx) < -boundary
                    predicted(idx,1) = 0;
                else
                    predicted(idx,1) = predicted(idx-1,1);
                end
                
                 predicted(idx,1) = (0.5*exp(-(certainty/2)*(predicted(idx,1)-1)^2))/(0.5*exp(-(certainty/2)*(predicted(idx,1)-1)^2)+(1-0.5)*exp(-(certainty/2)*(predicted(idx,1)-0)^2));
