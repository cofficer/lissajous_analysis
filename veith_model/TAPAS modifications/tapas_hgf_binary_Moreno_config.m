...
c.log_tau_s_Mu = [log(50)];
c.log_tau_s_Sa = [0];

c.log_noise_Mu = [log(0.1)];
c.log_noise_Sa = [1];

c.log_sigma_Mu = [log(0.7)];
c.log_sigma_Sa = [0];

c.log_boundary_Mu = [log(0.05)];
c.log_boundary_Sa = [0];

c.log_certainty_Mu = [log(10)];
c.log_certainty_Sa = [1];
...
expectedLength = 3*c.n_levels+2*(c.n_levels-1)+1+6;
...
c.prc_fun = @tapas_hgf_binary_Moreno;

% Handle to function that transforms perceptual parameters to their native space
% from the space they are estimated in
c.transp_prc_fun = @tapas_hgf_binary_transp_Moreno;
