...
% is centered at 0.
c.mu_0mu = [NaN, 0, 1, NaN, 0, 1];
c.mu_0sa = [NaN, 0, 0, NaN, 0, 0];

c.logsa_0mu = [NaN,   log(4), log(4), NaN,   log(4), log(4)];
c.logsa_0sa = [NaN,          1,      1, NaN,          1,      1];

% Rhos
% Format: row vector of length n_levels.
% Undefined (therefore NaN) at the first level.
% Fix this to zero to turn off drift.
c.rhomu = [NaN, 0, 0, NaN, 0, 0];
c.rhosa = [NaN, 0, 0, NaN, 0, 0];
...
c.logkamu = [NaN, log(1), NaN, NaN, log(1)];
c.logkasa = [NaN,      0, NaN, NaN,      0];

% Omegas
% Format: row vector of length n_levels.
% Undefined (therefore NaN) at the first level.
c.ommu = [NaN,  -3,  -6, NaN,  -3,  -6];
c.omsa = [NaN, 0, 0, NaN, 0, 0];

c.s_learning_mu = [log(0.5)];
c.s_learning_sa = [(10)];

c.s_priming_mu = [log(0.1)];
c.s_priming_sa = [(10)];
...
c.prc_fun = @tapas_hgf_binary_v_mixed;

% Handle to function that transforms perceptual parameters to their native space
% from the space they are estimated in
c.transp_prc_fun = @tapas_hgf_binary_mixed_transp;
