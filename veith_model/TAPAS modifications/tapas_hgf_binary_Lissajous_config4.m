...
c.logInitPiMu = [log(0.5)];
c.logInitPiSa = [5];

c.logErrorPiMu = [log(0.1)];
c.logErrorPiSa = [0];

c.logStereoPiMu = [log(5)];
c.logStereoPiSa = [5];
...
expectedLength = 3*c.n_levels+2*(c.n_levels-1)+1+3;
...
c.prc_fun = @tapas_hgf_binary_Lissajous;

% Handle to function that transforms perceptual parameters to their native space
% from the space they are estimated in
c.transp_prc_fun = @tapas_hgf_binary_transp_Lissajous;
