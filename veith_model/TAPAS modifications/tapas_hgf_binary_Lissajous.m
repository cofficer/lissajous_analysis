...
if ~isempty(varargin) && strcmp(varargin{1},'trans');
    p = tapas_hgf_binary_transp_Lissajous(r, p);
end
...
for k = 2:1:n
    if not(ismember(k-1, r.ign))
        ...
        if InitPi~=0
            if PerceptualAge(k,1)==1
                Precision(k,1)=InitPi;
            else
                %Precision(k,1)=Precision(k-1,1)*exp(-abs(daq(k-1))*(ErrorPi/InitPi));
                Precision(k,1)=Precision(k-1,1)*exp(-abs(daq(k-1)));
        ...
        %% StereoCue
        Precision(k,2)=StereoPi; % Stereocue
        Prediction(k,2)=u(k,3); % Stereocue
        ...
        daq(k,1) = u(k,1) -q(k,1);
        if k ~= 2
        w_daq(k,1) = daq(k)*(ErrorPi/Precision(k,1));
        else
        w_daq(k,1) = 0; 
        end
        %% Perceptual Age
          if k==2, NewPerceptualDecision(k,1)=1;
          else
        ...
        PerceptualAge(k,:)=PerceptualAge(k-1,:);
        NewPerceptualDecision(k,:)=NewPerceptualDecision(k-1,:);
